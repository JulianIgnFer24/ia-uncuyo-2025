...
**Subcuadros:** El tablero se divide en cajas de $k \times k$ (con $k^2 = N$).  
...
No hay función objetivo de optimización: es un **problema de satisfacción pura**.
...
### 1) Efecto de $WA = red$
- Revisar $(NT, WA)$: Como $WA$ solo permite *red*, eliminamos *red* de $D(NT)$.  
  → $D(NT) = \{green, blue\}$

- Revisar $(SA, WA)$: Eliminamos *red* de $D(SA)$.  
  → $D(SA) = \{green, blue\}$

### 2) Efecto de $V = blue$
- Revisar $(SA, V)$: Quitamos *blue* de $D(SA)$.  
  → $\boxed{D(SA) = \{green\}}$ (**dominio unitario**)

- Revisar $(NSW, V)$: Quitamos *blue* de $D(NSW)$.  
  → $D(NSW) = \{red, green\}$

Al volverse unitario $SA$, AC-3 vuelve a encolar los arcos $(Z, SA)$ para todos sus vecinos: $Z \in \{WA, NT, Q, NSW, V\}$.

### 3) Propagar $SA = green$
- $(NT, SA)$: eliminar *green* → $\boxed{D(NT) = \{blue\}}$
- $(Q, SA)$: eliminar *green* → $D(Q) = \{red, blue\}$
- $(NSW, SA)$: eliminar *green* → $D(NSW) = \{red\}$
- $(WA, SA)$ y $(V, SA)$: sin cambios (ya distintos de *green*).

Al volverse unitarios $NT$ y $NSW$, se re-encolan $(Z, NT)$ y $(Z, NSW)$ para sus vecinos.

### 4) Propagar $NT = blue$
- $(Q, NT)$: eliminar *blue* → $\boxed{D(Q) = \{red\}}$
- $(WA, NT)$ y $(SA, NT)$: sin cambios (ya distintos).

Al volverse unitario $Q$, AC-3 encola $(Z, Q)$ para sus vecinos: $Z \in \{NT, SA, NSW\}$.

### 5) Propagar $Q = red$
- $(NSW, Q)$: eliminar *red* de $D(NSW)$.  
  Pero $D(NSW) = \{red\}$ → **Se vacía:** $\boxed{D(NSW) = \emptyset}$
...
En un CSP estructurado como árbol, donde el grafo de restricciones forma un árbol (cualquier dos variables están relacionadas por **a lo sumo un camino**), la complejidad en el peor caso de AC-3 es **$O(ed^2)$**, donde:
- $e$ = número de arcos (en un árbol con $n$ variables, $e = n-1$)
- $d$ = tamaño máximo de los dominios

### ¿Por qué esta complejidad?

**Revisar un arco $(X_i, X_j)$** requiere:
- Verificar cada valor en $D(X_i)$ (hasta $d$ valores)
- Para cada uno, comprobar si existe al menos un valor consistente en $D(X_j)$ (hasta $d$ comparaciones)
- **Costo por revisión:** $O(d^2)$

En un **árbol**, cada arco se revisa **exactamente una vez**:
- Total de arcos: $e = n-1$
- **Complejidad total:** $O(ed^2)$
